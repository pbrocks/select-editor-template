...
namespace SelectEditorTemplate;
...
function enqueue_editor_assets() { // phpcs:ignore
	wp_register_script(
		...
		plugins_url( '/dist/build.js', dirname( __FILE__ ) ),
		array(
			'wp-blocks',
			'wp-i18n',
			'wp-element',
		...
		filemtime( plugin_dir_path( __DIR__ ) . 'dist/build.js' ),
		true
	);
	...
	if ( is_admin() && 'post' === get_current_screen()->id ) {
		...
		if ( $template ) {
			wp_localize_script( 'select-editor-template', 'SelectEditorTemplate', array(
	...
	wp_enqueue_style( 'select_template-panel', plugins_url( 'dist/style.build.css', dirname( __FILE__ ) ) );
}
